...
def split_train_test(X: pd.DataFrame, y: pd.Series, train_proportion: float = .25) \
    ...
    """
    n_samples = X.shape[0]  # num of samples
    train_X = X.iloc[0:math.ceil(train_proportion * n_samples)]
    train_y = y.iloc[0:math.ceil(train_proportion * n_samples)]
    test_X = X.iloc[math.floor(train_proportion * n_samples):n_samples]
    test_y = y.iloc[math.floor(train_proportion * n_samples):n_samples]
