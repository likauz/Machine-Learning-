...
from ...metrics import mean_square_error, misclassification_error
...
class Perceptron(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        if self.include_intercept_:
            x_tmp = np.hstack((X, np.ones((X.shape[0], 1))))
        self.coefs_ = np.zeros(x_tmp.shape[1])
        for i in range(self.max_iter_):
            cur_matrix = x_tmp @ self.coefs_
            indicates = False
            for j in range(len(cur_matrix)):
                if y[j] * cur_matrix[j] <= 0:
                    self.coefs_ += y[j] * x_tmp[j]
                    self.callback_(self, x_tmp[j], y[j])
                    indicates = True
                    break
            if not indicates:
                return
    ...
    def _predict(self, X: np.ndarray) -> np.ndarray:
        ...
        if self.include_intercept_:
            x_tmp = np.hstack((X, np.ones((X.shape[0], 1))))
        return np.sign(x_tmp @ self.coefs_)
