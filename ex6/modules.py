...
class LogisticModule(BaseModule):
    ...
    def compute_output(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        """
        n_samples = len(X)
        return ((-1) * np.sum(y * (X @ self.weights) - np.log(1 + np.exp(X @ self.weights)))) / n_samples
    ...
    def compute_jacobian(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        """
        exp = np.exp(X @ self.weights)
        n_samples = len(X)
        value = np.zeros(X.shape[1])
        for i in range(n_samples):
            value += (X[i] * exp[i]) / (exp[i] + 1)
        return (-1 / len(X)) * ((y @ X) - value)
...
class RegularizedModule(BaseModule):
    ...
    def compute_jacobian(self, **kwargs) -> np.ndarray:
        ...
        return regularization * self.lam_ + self.fidelity_module_.compute_jacobian(**kwargs)
